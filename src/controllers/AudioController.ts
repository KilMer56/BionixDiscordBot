    ...
    async joinVoiceChannel(message: Discord.Message) {
        if (message.member.voiceChannel) {
            if (!message.member.voiceChannel.joinable) {
                ...
                DiscordUtils.displayText(message, 'I have successfully connected to the channel!');
            }
        }
        else {
            DiscordUtils.displayText(message, 'You need to join a voice channel first!');
        }
    }
    ...
    leaveVoiceChannel(message: Discord.Message) {
        if (message.member.voiceChannel) {
            if (message.guild.me.voiceChannelID !== message.member.voiceChannelID) {
                DiscordUtils.displayText(message, 'I\'m not in the same channel !');
            }
            else {
                try {
